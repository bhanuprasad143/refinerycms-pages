module Refinery
  ...
  protected
    ...
    def current_refinery_user_can_access?(plugin)
      current_user? && current_user.authorized_plugins.include?(plugin)
    end
    ...
    def find_page(fallback_to_404 = true)
      @page ||= case action_name
                when "home"
                  Refinery::Page.by_site(current_site).where(:link_url => '/').first
                when "show", "preview"
                  Refinery::Page.by_site(current_site).find_by_path_or_id(params[:path], params[:id])
                end
